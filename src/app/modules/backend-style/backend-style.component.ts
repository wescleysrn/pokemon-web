...
@Component({
  selector: 'app-backend-style',
  templateUrl: './backend-style.component.html',
  styleUrls: ['./backend-style.component.scss']
})
export class BackendStyleComponent implements OnInit {
  ...
  public getPokemonsByCityTemperature() {
    this.backendService.getWeatherByCityName(this.city).subscribe(data => {
      this.wearther = data as CityWeatherModel;
      this.backendService.getPokemonByType(this.getPokemonType(this.wearther.main.temp)).subscribe(data => {
        this.pokemons = data;
  ...
  public loadPokemonImage() {
    ...
    this.backendService.getPokemonImageByUrl(this.pokemon.pokemon[0].url).subscribe(data => {
